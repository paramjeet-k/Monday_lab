...
class FundRaising:
    ...
    def raise_funds(self, ipo_amount=0, pe_amount=0, debt_amount=0, ps_amount=0):
        remaining_needed = self.project_cost - self.funds_raised
        
        if remaining_needed <= 0:
            return
        
        for source, amount in [('IPO', ipo_amount), ('Private Equity', pe_amount), ('Debt', debt_amount), ('Preference Shares', ps_amount)]:
            if remaining_needed > 0:
...
dept_amount = st.number_input("Maximum Debt Amount (INR):", min_value=0.0, format="%.2f")
...
if st.button("Calculate Fundraising Plan"):
    fund_raising.raise_funds(ipo_amount, pe_amount, dept_amount, ps_amount)
